...
export default function PublisherPostUnpublishIndex(
	...
	const createMenuItems = (post:Post) => {
		...
		items.push({
			...
			onClick: () => {
				setLoading(true)
				axios.post('/publisher/posts-publish/' + post.id).then(res=>{
					if(res.data.status === 'publish'){
						modal.info({
							title: 'Publish!',
							content: 'Successfully set to publish.'
						})
			...
			onClick: () => {
				...
				axios.post('/publisher/posts-return-to-author/' + post.id).then(res=>{
					if(res.data.status === 'return'){
						modal.info({
							title: 'Returned!',
							content: 'Successfully set to returned to author.'
						})
						loadAsync(search, perPage, page)
						setLoading(false)
					}
	...
	const loadAsync = async (
		...
		try{
			const res = await axios.get<PostResponse>(`/publisher/get-post-unpublish?${params}`);
		...
		<>
			...
			<div className='flex justify-center items-center'>
				...
				<div className='p-6 w-[1320px] overflow-auto mx-2 bg-white shadow-sm rounded-md
					...
					md:w-[1200px]'>
					{/* card header */}
					<div className="font-bold text-lg mb-4">LIST OF UNPUBLISH ARTICLES</div>
					{/* card body */}
...
PublisherPostUnpublishIndex.layout = (page:any) => <Authenticated user={page.props.auth.user}>{page}</Authenticated>
