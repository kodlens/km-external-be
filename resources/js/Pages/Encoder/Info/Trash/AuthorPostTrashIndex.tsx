...
export default function AuthorTrashIndex(
	...
	const createMenuItems = (data:Post) => {
		...
		items.push({
			...
			onClick: () => {
				setLoading(true)
				axios.post('/author/posts-draft/' + data.id).then(res=>{
					if(res.data.status === 'draft'){
						modal.info({
							title: 'Draft!',
							content: 'Successfully set to draft.'
						})
	...
	const loadAsync = async (
		...
		try{
			const res = await axios.get<PostResponse>(`/author/get-post-trashes?${params}`);
		...
		<Authenticated user={auth.user}>
			...
			<div className='flex mt-10 justify-center items-center'>
				...
				<div className='p-6 w-[1320px] overflow-auto mx-2 bg-white shadow-sm rounded-md
					...
					md:w-[1200px]'>
					{/* card header */}
					<div className="font-bold text-lg mb-4">LIST OF TRASH ARTICLES</div>
					{/* card body */}
